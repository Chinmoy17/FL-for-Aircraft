...
The security-side research question, run with the same FD001+FD003 / 4
clients / 50 rounds / seed=42 setup as P6 + RQ2 + the trilogy.

### Threat model

One malicious airline (`client_3` on FD003) wants the global model to
under-predict failures on FD001 engines (the competitor's fleet). The
server only sees weight tensors — it cannot inspect client data and
cannot tell honest updates from poisoned ones. Defenses must operate on
the update signal alone.

Two attacks tested:

| Attack | Mechanism |
| --- | --- |
| **A1 Label flip** | Locally invert RUL labels (RUL → 125 − RUL), re-derive fault, train honestly on the lie. Magnitude looks normal. |
| **A2 Gradient scale (×−10)** | Train honestly, then send `W_global + (−10)·(W_local − W_global)`. Update magnitude is 10× honest. |

Three Byzantine-robust aggregators tested:

| Aggregator | Mechanism | Reference |
| --- | --- | --- |
| **D1 Trimmed mean** (β=0.25) | Per parameter, drop lowest+highest, average middle | Yin et al. ICML 2018 |
| **D2 Coordinate-wise median** | Per parameter, median across clients | Yin et al. 2018 |
| **D3 Krum** (f=1) | Pick the single client whose flattened update is closest to its n−f−2 neighbors; that client's full state becomes the new global | Blanchard et al. NeurIPS 2017 |

### 11-cell matrix (RMSE on combined FD001+FD003 test set, lower is better)

| Cell | Attack | Defense | RMSE | F1 | Δ vs B0 baseline |
|---|---|---|---:|---:|---:|
| B0 | — clean | vanilla FedAvg | **17.95** | 0.871 | baseline |
| B1 | — clean | trimmed mean | 17.56 | 0.871 | −0.39 (sanity) |
| B2 | — clean | Krum (f=1) | 18.71 | 0.773 | +0.76 |
| **AV1** | label flip | vanilla | **29.92** | 0.467 | **+11.97** |
| **AV2** | grad ×−10 | vanilla | **84.03** | 0.000 | **+66.08** |
...
**1. Krum dominates both attacks identically (RMSE 19.80 / F1 0.779).**
Once `client_3` is geometrically isolated by the n−f−2 nearest-
neighbor distance check, it never contributes to the global model
again. The 1.85-RMSE gap to the clean baseline is the only cost. The
defense is essentially perfect.

**2. Gradient scaling is catastrophic when undefended (RMSE 84.03,
F1=0.000).** The model collapses to a near-constant prediction across
all engines and flags no faults. For a real maintenance pipeline this
would ground zero aircraft when failures are imminent — the worst-case
operational consequence is asymmetric and severe.

**3. The attacker is not subtle — the delta-norm plot is a smoking gun.**
On log scale, `client_3`'s update L2 norm sits at ~60 in round 1 and
stays above 100 for 20 rounds, while honest clients hover between 0.1
and 10. The order-of-magnitude separation exactly matches the scale=−10
multiplier. Any outlier detector on update norms catches this trivially.

**4. Trimmed mean = median when n=4 (mathematical, not a bug).** Cells
D11/D12 and D21/D22 produced bit-identical numbers because for n=4
with β=0.25, trimmed mean averages the middle 2 of 4 sorted values per
parameter, which equals (sorted[1] + sorted[2])/2 — exactly the median
formula for even n. The two aggregators only diverge for n ≥ 5. This is
a degenerate-case finding recorded transparently in the writeup.

### Why this extends Landau et al. (2026)

The brief's reference [10] (Landau et al., 2026) addressed PHM federated
learning with **accidentally** corrupted updates (malfunctioning sensors,
noisy clients). Their recommended defense was robust aggregation. The
framing is correct but the threat model is too benign — noise-robust
aggregators are tuned against well-behaved randomness, not against an
attacker who knows you're filtering.

RQ7 extends Landau's framework to **deliberately crafted poisoning**:
...
- The geometric whole-update check (Krum) is the intervention that
  actually works against adversaries.

### Operational takeaway

For a real multi-airline consortium, the default vanilla FedAvg server
is operationally unsafe. The fix is one line: swap `fedavg_aggregate`
for `make_krum_aggregator(num_byzantine=1)`. The cost is 0.76 RMSE
regression in the clean case (18.71 vs 17.95) — well worth the
1.85-RMSE worst-case ceiling under attack vs the alternative of 84-RMSE
collapse.
...
| Component | Role | LOC |
| --- | --- | --- |
| [`src/fl_aircraft/fl/poisoning.py`](src/fl_aircraft/fl/poisoning.py) | `MaliciousClient` ABC + `LabelFlipAttacker` + `GradientScaleAttacker` + `_LabelFlippedDataset` | ~280 |
| [`src/fl_aircraft/fl/robust_aggregators.py`](src/fl_aircraft/fl/robust_aggregators.py) | `make_trimmed_mean_aggregator`, `make_median_aggregator`, `make_krum_aggregator` | ~220 |
| [`src/fl_aircraft/fl/poisoned_simulation.py`](src/fl_aircraft/fl/poisoned_simulation.py) | `run_fedavg_with_attackers` + `PoisonedHistory` with per-client delta-norm tracking | ~280 |
| [`tests/test_rq7.py`](tests/test_rq7.py) | 12 unit tests (attackers + aggregators) | ~280 |
| [`scripts/run_rq7.py`](scripts/run_rq7.py) | 11-cell matrix CLI + 4 plots | ~510 |
| [`frontend/src/pages/Rq7StoryPage.tsx`](frontend/src/pages/Rq7StoryPage.tsx) | Long-form `/rq7-story` page | ~330 |
...
- **Backdoor attacks** — attacker injects a trigger pattern into specific
  (cycle, sensor) cells and labels those windows as max-healthy. The
  model learns correct predictions on clean inputs (passes standard
  tests) but always-healthy on triggered inputs. Krum may not defend if
  multiple compromised clients coordinate the trigger.
- **Coordinated attackers** — Krum's f=1 setting tolerates only 1
  Byzantine. With 6+ clients and 2 coordinated attackers, the
  "geometrically isolated" argument breaks because the attackers can
  cluster together. FoolsGold and similar coordination-aware defenses
  are the natural next layer.
- **Poisoning under FedRep** — if heads stay local (FedRep architecture),
  the attacker can only poison the encoder. Each honest client's personal
  head still maps degradation features correctly. The blast radius should
  be much smaller. Worth running as a 12th cell.
- **RQ6 link** — a defense that inspects update structure (Krum) leaks
  information about client data via that same structure. RQ6 quantifies
  the privacy cost. The two security questions interlock: defenses
  against poisoning may weaken privacy, and vice versa.
